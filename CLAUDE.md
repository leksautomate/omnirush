...
|---|---|---|
| `app-live/` | The product: a Next.js 16 chat app (forked from [miurla/morphic](https://github.com/miurla/morphic)) extended with an agentic video-generation pipeline. Has its own `CLAUDE.md` and `AGENTS.md` — read those before working inside this directory. | Vercel |
...
| `hook/` | An unrelated **HyperFrames** composition project (short-form video "hook" HTML compositions). Has its own `CLAUDE.md`/`AGENTS.md` describing the HyperFrames authoring workflow — a different framework from `app-live`'s Remotion pipeline. Do not conflate the two. |
...
For nearly all day-to-day work you'll be inside `app-live/` — see `app-live/CLAUDE.md` for its
